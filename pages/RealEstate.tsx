...
const RealEstatePage: React.FC = () => {
   ...
   const handleSaveImovel = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const data: Imovel = {
         ...
         documentacao_status: fd.get('doc_status') as any,
         comodidades: fd.get('comodidades')?.toString().split(',').map(s => s.trim()).filter(Boolean) || []
      };
               ...
               <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
                  <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
                     ...
                     <form onSubmit={handleSaveImovel} className="p-10 space-y-6 max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="p-6 bg-yellow-50 rounded-3xl border border-yellow-100 space-y-4">
                           <Select name="doc_status" label="Estado da Documentação" defaultValue={editingImovel?.documentacao_status} options={[
                              { value: 'Escritura', label: 'Escritura Pública' },
                              { value: 'IPU Pendente', label: 'IPU Pendente' },
                              { value: 'Legalizado', label: 'Legalizado' },
                              { value: 'Em Processo', label: 'Em Processo de Legalização' }
                           ]} />
                           <div className="space-y-2">
                              <label className="text-xs font-black text-zinc-400 uppercase tracking-widest">Comodidades (Separadas por vírgula)</label>
                              <textarea
                                 name="comodidades"
                                 defaultValue={editingImovel?.comodidades?.join(', ')}
                                 className="w-full bg-white border border-zinc-200 rounded-xl p-3 text-sm outline-none focus:border-yellow-500 min-h-[60px]"
                                 placeholder="Ex: Piscina, Garagem, Segurança 24h, AC"
                              />
                           </div>
                        </div>
