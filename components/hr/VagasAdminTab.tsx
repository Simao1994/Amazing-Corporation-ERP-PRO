...
const VagasAdminTab: React.FC = () => {
    ...
    const fetchVagas = async () => {
        ...
        const { data, error } = await safeQuery(() =>
            supabase.from('rh_vagas').select('*').eq('tenant_id', user.tenant_id).order('criado_em', { ascending: false })
        );
        if (error) {
            console.error(error);
        ...
        try {
            if (editingVaga.id) {
                ...
                const { error } = await safeQuery(() =>
                    supabase.from('rh_vagas').update(editingVaga).eq('id', editingVaga.id).eq('tenant_id', user.tenant_id)
                );
                ...
                const { error } = await safeQuery(() =>
                    supabase.from('rh_vagas').insert([{ ...vagaToInsert, tenant_id: user.tenant_id }])
                );
                if (error) throw error;
            }
