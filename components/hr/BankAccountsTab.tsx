...
import { PlusCircle, Edit, Trash2, Building, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react';
...
const BankAccountsTab: React.FC<BankAccountsTabProps> = ({ funcionarioId, user }) => {
   ...
   const fetchContas = async () => {
      ...
      try {
         ...
         // Se a tabela ainda não existir no bd (antes de rodarem o script SQL), supabase retorna erro 42P01. 
         // Mostramos apenas uma lista vazia pra não rebentar a UI.
         setContas([]);
      } finally {
         setLoading(false);
      }
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         ...
         const payLoad = {
            ...
            iban: formatIBAN(formData.iban), // Normaliza formatação
   ...
   const setAsPrincipal = async (id: string) => {
      try {
         await supabase.from('rh_contas_bancarias').update({ principal: false }).eq('funcionario_id', funcionarioId);
               ...
               <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-end gap-4 pt-6 border-t border-zinc-100">
                     ...
                     <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-zinc-900 text-white font-black text-xs uppercase rounded-xl hover:bg-yellow-500 hover:text-zinc-900 transition-all shadow-lg flex items-center gap-2 disabled:opacity-50">
                        {isSubmitting ? <RefreshCw size={16} className="animate-spin" /> : <SaveIcon />} Guardar Conta
                     </button>
                  </div>
               </form>
            </div>
